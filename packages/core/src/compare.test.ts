...
// Test utilities
function createSolidColorPNG(width: number, height: number, color: [number, number, number, number] = [255, 0, 0, 255]): Buffer {
...
function createCheckerboardPNG(width: number, height: number, squareSize: number = 10): Buffer {
  const png = new PNG({ width, height });
...
describe("compare", () => {
  ...
  describe("compareImages", () => {
    ...
    it("should apply threshold option correctly", async () => {
      ...
      expect(strictResult.numDiffPixels).toBeGreaterThan(lenientResult.numDiffPixels);
    });
  ...
  describe("imagesMatch", () => {
    ...
    it("should pass through compare options", async () => {
      ...
      const strictMatch = await imagesMatch(image1, image2, 50, { threshold: 0.01 });
      const lenientMatch = await imagesMatch(image1, image2, 50, { threshold: 0.5 });
  ...
  describe("error handling", () => {
    it("should handle non-existent file paths", async () => {
      ...
      await expect(compareImages(nonExistentPath, validImage)).rejects.toThrow();
    });
