...
| `diff` | Override the global diff configuration for this story (threshold, AA, pixel/percentage limits). | Inherits global `diff` from `cappa.config.ts` |
...
export const Primary = {
  parameters: {
    cappa: {
      ...
      mask: ['.tooltip'],
      diff: {
        // Loosen comparison slightly for this story only
        threshold: 0.2,
      ...
      variants: [
        {
          ...
          options: {
            ...
            // Per-variant diff override, merged on top of the global and story-level diff
            diff: {
              threshold: 0.25,
            },
          },
