...
Runs after a capture completes if there are screenshots that failed comparison. The callback receives
an array of failing screenshots, including both relative and absolute paths to the actual, expected,
and diff images. Use this hook to upload diff images to object storage or integrate with CI systems.
