...
At most five regions are listed per screenshot; the rest are collapsed into a
`… and N more regions` line. Because the report is printed to stdout, it gives you the *where* and
*what* of a diff directly in your CI logs, without having to download the diff artifacts or open
the review UI.
...
[`cappa capture`](#change-report) prints on failure.
