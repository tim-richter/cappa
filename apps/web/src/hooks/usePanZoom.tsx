...
function clampTranslate(
	...
	const minClampX = canCenterX ? Math.min(minX, (container.width - w) / 2) : minX
	const maxClampX = canCenterX ? Math.max(maxX, (container.width - w) / 2) : maxX
	
	const minClampY = canCenterY ? Math.min(minY, (container.height - h) / 2) : minY
	const maxClampY = canCenterY ? Math.max(maxY, (container.height - h) / 2) : maxY
	...
	useEffect(() => {
		...
		function onPointerMove(e: PointerEvent) {
			...
			if (pts.size >= 2) {
				...
				zoomTo(nextScale, { x: mid.x - el.getBoundingClientRect().left, y: mid.y - el.getBoundingClientRect().top })
				return
			}
